// scripts/create-clean-test-user.js
// Temiz test kullanıcısı oluşturma scripti
// MongoDB'de olması gereken verileri içerir (PostgreSQL snapshot'ları içermez)
...
async function createCleanTestUser() {
  try {
    ...
    const existingUser = await User.findOne({ email: 'test@fintech.com' });
    if (existingUser) {
      console.log('🗑️  Deleting existing test user...');
      await User.deleteOne({ email: 'test@fintech.com' });
    ...
    const testUser = new User({
      ...
      phone: '+905551234567',
      cumulativeSavings: 45000, // 6 ayda biriken tasarruf
      ...
      finance: {
        ...
        // Değişken Giderler (Toplam: ~12,000 TL)
        variableExpenses: [
    ...
    console.log('\n✅ Clean test user created successfully!');
    console.log('\n📋 User Details:');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('\n💰 Financial Summary:');
    console.log(`   Monthly Income: ₺${testUser.finance.monthlyIncome.toLocaleString()}`);
    console.log(`   Fixed Expenses: ₺${testUser.finance.fixedExpenses.reduce((sum, e) => sum + e.amount, 0).toLocaleString()}`);
    console.log(`   Variable Expenses: ₺${testUser.finance.variableExpenses.reduce((sum, e) => sum + e.amount, 0).toLocaleString()}`);
    console.log(`   Monthly Savings: ₺${(testUser.finance.monthlyIncome - testUser.finance.fixedExpenses.reduce((sum, e) => sum + e.amount, 0) - testUser.finance.variableExpenses.reduce((sum, e) => sum + e.amount, 0)).toLocaleString()}`);
    console.log(`   Cumulative Savings: ₺${testUser.cumulativeSavings.toLocaleString()}`);
    console.log('\n💳 Credit Cards:');
    testUser.creditCards.forEach(card => {
      console.log(`   ${card.bankName}: ₺${card.currentDebt.toLocaleString()} / ₺${card.limit.toLocaleString()} (${card.utilizationRate}%)`);
    });
    console.log('\n🏦 Debts:');
    testUser.debts.forEach(debt => {
      console.log(`   ${debt.name}: ₺${debt.remainingAmount.toLocaleString()} remaining`);
    });
    console.log('\n📊 Investments:');
    testUser.investments.forEach(inv => {
      console.log(`   ${inv.name}: ₺${inv.currentValue.toLocaleString()} (${inv.profitLoss >= 0 ? '+' : ''}₺${inv.profitLoss.toLocaleString()})`);
    });
    console.log('\n🏠 Assets:');
    testUser.assets.forEach(asset => {
    ...
    console.log('⚠️  IMPORTANT: PostgreSQL snapshots are NOT created.');
    console.log('   Use "Yeni Aya Geç" button to generate monthly snapshots.');
    console.log('   Or run the snapshot migration script separately.\n');
